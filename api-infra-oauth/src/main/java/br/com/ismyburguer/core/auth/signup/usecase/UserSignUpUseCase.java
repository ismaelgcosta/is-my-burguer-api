...
public class UserSignUpUseCase {
    ...
    public UserSignUpUseCase(CadastrarClienteUseCase cadastrarClienteUseCase, ConsultarClienteUseCase consultarClienteUseCase, OAuthSignUpUseCase oAuthSignUpUseCase) {
    ...
    public Cliente cadastrarNovoUsuario(Cliente cliente, @NotBlank(message = "Informe o password") String password) {
        ...
        oAuthSignUpUseCase.signUp(new SignUpRequest(
                username,
                password,
    ...
    private static String generateUsername(Cliente.Nome nome) {
        String validNumber = new Faker().idNumber().valid();
        return StringUtils.lowerCase(nome.getNome() + (nome.getSobrenome() == null ? "" : nome.getSobrenome()) + validNumber.split("-")[2]);
    }
}
